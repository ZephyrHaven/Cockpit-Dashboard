...
class CockpitRssService {
  ...
  async saveConfig(next) { this.config = normalizeRssConfig(next); const data = await this.plugin.loadData() || {}; data.calendarRss = this.config; await this.plugin.saveData(data); return this.config; }
  async _db() {
  ...
  async _prune(persist = false) {
    ...
    const kept = this._items.slice().sort((a,b) => b.publishedAt - a.publishedAt).filter((item) => {
      ...
      if (item.publishedAt < cutoff || count >= RSS_LIMITS.perFeed || bytes + size > RSS_LIMITS.maxBytes || total >= RSS_LIMITS.maxItems) return false;
  ...
  async refresh(force = false) {
    ...
    const byId = new Map(this._items.map((item) => [item.id, item])); incoming.forEach((item) => { const previous = byId.get(item.id); byId.set(item.id, previous?.readAt ? { ...item, readAt:previous.readAt } : item); }); this._items = Array.from(byId.values()); await this._prune(true); this._lastRefresh = Date.now();
  ...
  async _fetchFeed(feed) {
    const response = await obsidian.requestUrl({ url:feed.url, method:'GET', headers:{ Accept:'application/rss+xml, application/atom+xml, application/xml, text/xml' }, throw:false });
...
class CockpitRssModal extends obsidian.Modal {
  constructor(app, view, date) { super(app); this.view = view; this.date = date || window.moment(); this.selectedId = null; this.selectedFeedId = 'all'; this.articleCache = new Map(); this._articleRequest = 0; this._dragCleanup = null; this._preserveListScroll = false; this._speech = { itemId:null, chunks:[], index:0, status:'idle', rate:1, token:0, playlist:[], autoStartNext:false }; this._speechChunks = new Map(); this._speechUi = null; }
  onOpen() { this.modalEl.addClass(PLUGIN_ID + '-rss-modal'); this.contentEl.addClass(PLUGIN_ID + '-rss-modal-content'); this.render(); }
  render() {
    const previousList = this.contentEl.querySelector('.' + PLUGIN_ID + '-rss-list'); const savedListScroll = this._preserveListScroll ? previousList?.scrollTop : null; this._preserveListScroll = false; const service = this.view._rss; const allItems = service.itemsForDate(this.date); const feedIds = Array.from(new Set(allItems.map((item) => item.feedId))); if (this.selectedFeedId !== 'all' && !feedIds.includes(this.selectedFeedId)) this.selectedFeedId = 'all'; const items = this.selectedFeedId === 'all' ? allItems : allItems.filter((item) => item.feedId === this.selectedFeedId); const selected = items.find((item) => item.id === this.selectedId) || items[0]; this.selectedId = selected?.id || null; this._speech.playlist = items.map((item) => item.id); this.contentEl.empty();
    ...
    const filters = list.createDiv({ cls:PLUGIN_ID + '-rss-filters' }); const addFilter = (id, label, color) => { const button = filters.createEl('button', { cls:PLUGIN_ID + '-rss-filter' + (this.selectedFeedId === id ? ' active' : ''), text:label, attr:{ type:'button' } }); if (color) button.style.setProperty('--rss-color', color); button.onclick = () => { this.selectedFeedId = id; this.selectedId = null; this.render(); }; }; addFilter('all', this.view._lang() === 'en' ? 'All sources' : '全部订阅'); feedIds.forEach((id) => { const feed = service.getFeed(id); addFilter(id, feed?.name || 'RSS', feed?.color); });
    if (!items.length) { detail.createDiv({ cls:PLUGIN_ID + '-rss-empty', text:this.view._lang() === 'en' ? 'No subscription entries for this date.' : '这一天没有订阅内容。' }); return; }
    items.forEach((item) => { const feed = service.getFeed(item.feedId); const row = list.createDiv({ cls:PLUGIN_ID + '-rss-card' + (item.id === selected.id ? ' active' : '') + (!item.readAt ? ' unread' : ''), attr:{ role:'button', tabindex:'0', style:'--rss-color:' + (feed?.color || '#818cf8') } }); const cardHead = row.createDiv({ cls:PLUGIN_ID + '-rss-card-head' }); cardHead.createDiv({ cls:PLUGIN_ID + '-rss-source', text:feed?.name || 'RSS' }); cardHead.createSpan({ cls:PLUGIN_ID + '-rss-read-state ' + (!item.readAt ? 'unread' : 'read'), text:!item.readAt ? (this.view._lang() === 'en' ? 'Unread' : '未读') : (this.view._lang() === 'en' ? 'Read' : '已读') }); row.createDiv({ cls:PLUGIN_ID + '-rss-row-title', text:item.title || '无标题' }); row.createDiv({ cls:PLUGIN_ID + '-rss-row-meta', text:window.moment(item.publishedAt).format('YYYY-MM-DD HH:mm') + (feed?.tags?.length ? ' · ' + feed.tags.map((tag) => '#' + tag).join(' ') : '') }); row.createDiv({ cls:PLUGIN_ID + '-rss-row-summary', text:item.summary || (this.view._lang() === 'en' ? 'No summary' : '无内容摘要') }); const choose = () => { this._preserveListScroll = true; this.selectedId = item.id; this.render(); }; row.onclick = choose; row.onkeydown = (evt) => { if (evt.key === 'Enter' || evt.key === ' ') { evt.preventDefault(); choose(); } }; });
    ...
    const articleActions = articleFrame.createDiv({ cls:PLUGIN_ID + '-rss-article-actions' }); if (/^https?:\/\//i.test(selected.link)) { const open = articleActions.createEl('button', { cls:PLUGIN_ID + '-rss-open-original mod-cta', text:this.view._lang() === 'en' ? 'Open original' : '打开原文', attr:{ type:'button' } }); open.onclick = () => window.open(selected.link, '_blank', 'noopener'); }
    ...
    requestAnimationFrame(() => { if (savedListScroll !== null && savedListScroll !== undefined) list.scrollTop = savedListScroll; else list.querySelector('.' + PLUGIN_ID + '-rss-card.active')?.scrollIntoView({ block:'nearest' }); });
    if (!selected.readAt) service.markRead(selected.id).then((changed) => { if (changed && this.selectedId === selected.id) { this.view._refreshCalendarRef?.(); this._preserveListScroll = true; this.render(); } });
  }
  async _renderLinkedArticle(el, item, onReady) {
    ...
    try {
      const response = await obsidian.requestUrl({ url:item.link, method:'GET', headers:{ Accept:'text/html,application/xhtml+xml' }, throw:false });
      if (response.status < 200 || response.status >= 300) throw new Error('HTTP ' + response.status);
  ...
  _toggleSpeech(targetId) { const speech = this._speech; if (speech.itemId !== targetId) { const chunks = this._speechChunks.get(targetId) || []; if (!chunks.length) { new obsidian.Notice(this.view._lang() === 'en' ? 'Article text is still loading.' : '正文仍在加载，请稍候。'); return; } this._stopSpeech(); this._speech = { itemId:targetId, chunks, index:0, status:'idle', rate:speech.rate || 1, token:speech.token || 0, playlist:speech.playlist || [], autoStartNext:false }; } if (!this._speech.chunks.length) return; if (this._speech.status === 'playing') { window.speechSynthesis.pause(); this._speech.status = 'paused'; this._updateSpeechControls(); return; } if (this._speech.status === 'paused') { window.speechSynthesis.resume(); this._speech.status = 'playing'; this._updateSpeechControls(); return; } if (this._speech.status === 'completed') this._speech.index = 0; this._speakCurrent(); }
  _speakCurrent() { const speech = this._speech; if (!speech.chunks[speech.index]) { speech.status = 'completed'; this._updateSpeechControls(); return; } window.speechSynthesis.cancel(); const token = ++speech.token; const utterance = new SpeechSynthesisUtterance(speech.chunks[speech.index]); utterance.lang = /[\u3400-\u9fff]/.test(utterance.text) ? 'zh-CN' : 'en-US'; utterance.rate = speech.rate || 1; utterance.onstart = () => { if (token === speech.token) { speech.status = 'playing'; this._updateSpeechControls(); } }; utterance.onend = () => { if (token !== speech.token || speech.status !== 'playing') return; speech.index += 1; if (speech.index >= speech.chunks.length) { speech.status = 'completed'; this._updateSpeechControls(); if (this.view._rss.config.autoPlayNextArticle) this._playNextArticle(); } else this._speakCurrent(); }; utterance.onerror = (event) => { if (token === speech.token && event.error !== 'canceled' && event.error !== 'interrupted') { speech.status = 'idle'; this._updateSpeechControls(); } }; speech.status = 'playing'; window.speechSynthesis.speak(utterance); this._updateSpeechControls(); }
  ...
  _stopSpeech() { if (window.speechSynthesis) window.speechSynthesis.cancel(); this._speech.token += 1; this._speech.status = 'idle'; this._speech.index = 0; this._speech.autoStartNext = false; this._updateSpeechControls(); }
  ...
  _showMenu(evt) {
    const en = this.view._lang() === 'en'; const menu = new obsidian.Menu();
    ...
    menu.addItem((item) => item.setTitle(en ? 'Clear local RSS cache' : '清除本机 RSS 缓存').setIcon('trash-2').onClick(async () => { await this.view._rss.clearCache(); new obsidian.Notice(en ? 'Local RSS cache cleared.' : '本机 RSS 缓存已清除。'); this.view._refreshCalendarRef?.(); this.render(); }));
  ...
  onClose() { this._dragCleanup?.(); this._stopSpeech(); this.contentEl.empty(); this.modalEl.removeClass(PLUGIN_ID + '-rss-modal'); }
}

class CockpitRssSettingsModal extends obsidian.Modal {
  ...
  render() {
    const en = this.view._lang() === 'en'; const service = this.view._rss; const stats = service.cacheStats(); this.contentEl.empty(); this.contentEl.createEl('h2', { text:en ? 'RSS subscriptions' : 'RSS 订阅' }); this.contentEl.createDiv({ cls:PLUGIN_ID + '-rss-network-note', text:en ? 'RSS uses your network to fetch the feed addresses you add. Feed configuration syncs with the plugin; cached entries stay only on this device and never enter data.json or Obsidian Sync.' : '启用后会联网访问你添加的订阅地址。订阅配置会随插件同步；缓存仅保存在当前设备，不写入 data.json，也不参与 Obsidian Sync。' });
    new obsidian.Setting(this.contentEl).setName(en ? 'Enable RSS subscriptions' : '启用 RSS 订阅').addToggle((toggle) => toggle.setValue(this.config.enabled).onChange((value) => { this.config.enabled = value; }));
    new obsidian.Setting(this.contentEl).setName(en ? 'Refresh interval' : '刷新频率').setDesc(en ? 'Automatic refresh is skipped while the dashboard is hidden.' : '驾驶舱隐藏时不会自动刷新。').addDropdown((drop) => drop.addOptions({ 0:en ? 'Manual only' : '仅手动', 60:en ? 'Every hour' : '每小时', 360:en ? 'Every 6 hours' : '每 6 小时' }).setValue(String(this.config.refreshMinutes)).onChange((value) => { this.config.refreshMinutes = Number(value); }));
