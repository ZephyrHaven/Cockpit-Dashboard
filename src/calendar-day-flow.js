...
function renderCalendarDayFlow(options) {
  const { anchor, date, language, t, todosForDate, rssItems, automationItems, filter, onFilterChange, openTodoEditor, onTodoToggle, onTodoAlarm, hasLinkedTodoAlarm, openRss, onAutomationOpen, onAutomationRun, rerender, rss } = options;
  ...
  const counts = { all:flowItems.length, todo:todos.length, automation:automations.length, rss:articles.length };
  const filters = detail.createDiv({ cls:PLUGIN_ID + '-cal-flow-filters', attr:{ role:'tablist', 'aria-label':en ? 'Filter day items' : '筛选日期事项' } });
  [['all',en?'All':'全部'],['todo',en?'Tasks':'待办'],['automation',en?'Automations':'自动化'],['rss','RSS']].forEach(([id,label]) => {
  ...
  visibleItems.forEach((flow, index) => {
    ...
    time.createSpan({ text:todo
      ...
      : rssEntry ? 'RSS' : (automation.time || (en ? 'Event' : '事件')) });
    ...
    if (todo) {
      ...
      obs.setIcon(node, rssEntry ? 'rss' : (automation.kind === 'workflow' ? 'workflow' : 'zap'));
    }
...
if (typeof module !== 'undefined' && module.exports && typeof PLUGIN_ID === 'undefined') module.exports={summarizeCalendarRss};
