...
const SCHEDULED_EVENT_COOLDOWN_MS = 20000;
...
function scheduledTemplateText(template) {
...
// 事件任务与文件路径的匹配：folder 为空匹配任意路径，否则按前缀匹配。
function eventTaskMatchesPath(task, paths) {
  const folder = String(task?.schedule?.folder || '');
  const list = Array.isArray(paths) ? paths : [];
  if (!folder) return list.length > 0;
...
function normalizeScheduledSmtpMail(raw) {
  if (!raw || typeof raw !== 'object') return null;
  const recipients = (value) => Array.from(new Set((Array.isArray(value) ? value : String(value || '').split(/[,;\n]/))
    .map((item) => String(item || '').trim().toLowerCase()).filter((item) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(item)))).slice(0, 30);
  const accountId = String(raw.accountId || '').replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 64);
  const to = recipients(raw.to);
  if (!accountId || !to.length) return null;
  return {
    accountId,
    to,
    cc:recipients(raw.cc),
    bcc:recipients(raw.bcc),
...
function normalizeScheduledTasks(raw) {
  ...
  return (Array.isArray(raw) ? raw : []).slice(0, SCHEDULED_TASK_LIMIT).map((item) => {
    ...
    seen.add(id);
    const kind = SCHEDULED_TASK_KINDS.includes(item?.kind) ? item.kind : 'shell';
    const command = String(item?.command || '').trim().slice(0, kind === 'shell' ? 12000 : 4000);
    const smtp = kind === 'smtp-email' ? normalizeScheduledSmtpMail(item?.smtp) : null;
    if (kind === 'smtp-email' ? !smtp : !command) return null;
    const scheduleType = ['interval', 'daily', 'weekly', 'event'].includes(item?.schedule?.type) ? item.schedule.type : 'daily';
...
class ScheduledTaskService {
  ...
  stop() { this.started = false; this.listeners.clear(); this._unbindEventTriggers?.(); this._unbindEventTriggers = null; }
  _bindEventTriggers() {
    // 事件触发器：库内文件事件挂宿主事件注册表（随插件卸载自动解绑），
    // 待办/番茄钟事件走插件内部事件总线。文件事件统一防抖后批量匹配任务。
    try {
      ...
      const flush = () => {
        ...
        pendingPaths.clear();
        this.dispatchEventTrigger('file-saved', { paths }).catch((e) => console.warn('[Cockpit event trigger]', e));
      };
      ...
      this.plugin.registerEvent(this.plugin.app.vault.on('modify', onVaultChange));
      this.plugin.registerEvent(this.plugin.app.vault.on('rename', (file) => onVaultChange(file)));
      this._unbindVaultEvents = () => {
  ...
  _unbindEventTriggers() {
    this._unbindVaultEvents?.();
    this._unbindVaultEvents = null;
    this._unbindCockpitEvents?.forEach((off) => { try { off(); } catch (e) {} });
    this._unbindCockpitEvents = null;
  }
  async dispatchEventTrigger(eventName, payload = {}) {
    if (!this.started) return;
    const tasks = await this.load();
    const now = Date.now();
    tasks.forEach((task) => {
      if (!task.enabled || task.schedule.type !== 'event' || task.schedule.event !== eventName) return;
  ...
  async runTask(id, options = {}) {
    ...
    try {
      ...
      } else if (task.kind === 'append-daily') {
        stdout = await this.appendDailyNote(task.command);
      } else if (task.kind === 'create-todo') {
        stdout = await this.createTodoFromTemplate(task.command);
      } else if (task.kind === 'push') {
        stdout = await this.pushNotification(task.command);
      } else if (task.kind === 'smtp-email') {
        if (!this.plugin.smtpMail?.send) throw new Error('smtp-service-unavailable');
        const subject = scheduledTemplateText(task.smtp.subject);
        const body = scheduledTemplateText(task.smtp.body);
        await this.plugin.smtpMail.send(task.smtp.accountId, {
          to:task.smtp.to, cc:task.smtp.cc, bcc:task.smtp.bcc, subject, body,
          messageId:[task.id, options.scheduledAt || Date.now()].join('-')
        });
        stdout = 'SMTP email accepted for ' + task.smtp.to.length + ' recipient(s).';
      } else if (task.kind === 'workflow') {
        // 流程作为任务动作：复用时间计划与事件触发两种入口，结果汇总进审计日志。
        const workflow = this.plugin.workflows;
        if (!workflow || typeof workflow.run !== 'function') throw new Error('workflow-engine-unavailable');
        const outcome = await workflow.run(task.command, { trigger:options.trigger === 'event' ? 'event' : 'schedule' });
        if (outcome.busy) throw new Error('Workflow is already running.');
        if (!outcome.ok) throw new Error('Workflow finished with status: ' + outcome.status);
        stdout = 'Workflow ' + task.command + ' → ' + outcome.status + ' (' + Math.round(outcome.record.durationMs / 100) / 10 + 's)';
      } else {
  ...
  async appendDailyNote(template) {
    const text = scheduledTemplateText(template).slice(0, 4000);
    if (!text) throw new Error('empty-template');
    const dir = typeof DAILY_DIR === 'string' ? DAILY_DIR : '_daily';
    const path = dir + '/' + window.moment().format('YYYY-MM-DD') + '.md';
    const vault = this.plugin.app.vault;
    if (!vault.getAbstractFileByPath(dir)) await vault.createFolder(dir);
    const file = vault.getAbstractFileByPath(path);
    if (!file) {
      await vault.create(path, '# ' + window.moment().format('YYYY-MM-DD') + '\n\n' + text + '\n');
      return 'Created daily note: ' + path;
    }
    const content = typeof vault.read === 'function' ? await vault.read(file) : '';
    const separator = content && !content.endsWith('\n') ? '\n\n' : '\n';
    await vault.modify(file, content + separator + text + '\n');
    return 'Appended to daily note: ' + path;
  }
  async createTodoFromTemplate(template) {
    const text = scheduledTemplateText(template).replace(/[\r\n\0]+/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 180);
    if (!text) throw new Error('empty-template');
    if (typeof mutateTodos !== 'function' || typeof createTodoId !== 'function') throw new Error('todo-layer-unavailable');
    const outcome = await mutateTodos(this.plugin.app.vault, (todos) => {
      todos.push({ id:createTodoId(), text, tags:[], priority:'mid', dueDate:null, dueHasTime:false, done:false, created:window.moment(), doneDate:null });
      return true;
    });
    if (!outcome?.saved) throw new Error('todo-create-failed');
    // 打开的驾驶舱需要同步新待办：复用 Agent 工具的刷新链路（闹钟同步 + 视图重载）。
    if (typeof refreshCockpitAfterAgentMutation === 'function') {
      await refreshCockpitAfterAgentMutation(this.plugin, outcome.todos).catch((e) => console.warn('[Cockpit scheduler] dashboard refresh failed', e));
    }
    return 'Todo created: ' + text;
  }
  async pushNotification(template) {
    const message = scheduledTemplateText(template).slice(0, 500);
    if (!message) throw new Error('empty-template');
    const config = await this.plugin.serverChan?.getConfig?.();
    const channels = config?.channels || {};
    const ids = Object.keys(channels).filter((id) => channels[id]?.enabled);
    if (!ids.length) throw new Error('push-channel-unconfigured');
    const results = await Promise.allSettled(ids.map((id) => sendNotificationChannel(id, channels[id], 'Cockpit', message)));
    const failed = results.filter((result) => result.status === 'rejected').length;
    if (failed >= ids.length) throw new Error('push-failed');
    return 'Push sent via ' + (ids.length - failed) + '/' + ids.length + ' channel(s)';
  }
  logPath() {
...
async function openScheduledTaskEditor(view, existing, options = {}) {
  const en = view._lang() === 'en';
  ...
  const smtpAccounts = view._plugin.smtpMail?.listAccounts ? await view._plugin.smtpMail.listAccounts() : [];
  ...
  const draft = { ...draftRaw, schedule:scheduledTaskEditorSchedule(draftRaw.schedule), smtp:{ accountId:draftRaw.smtp?.accountId || smtpAccounts[0]?.id || '', to:draftRaw.smtp?.to || [], cc:draftRaw.smtp?.cc || [], bcc:draftRaw.smtp?.bcc || [], subject:draftRaw.smtp?.subject || '', body:draftRaw.smtp?.body || '' } };
  const overlay=document.createElement('div'); overlay.className=PLUGIN_ID+'-scheduler-backdrop';
  ...
  const kind=field(en?'Task type':'任务类型').createEl('select'); kind.createEl('option',{text:en?'Toolbar action':'Toolbar 动作',attr:{value:'toolbar-action'}}); kind.createEl('option',{text:en?'App command':'应用命令',attr:{value:'obsidian-command'}}); kind.createEl('option',{text:en?'Shell command (desktop)':'Shell 命令（仅桌面端）',attr:{value:'shell'}}); kind.createEl('option',{text:en?'Send SMTP email':'发送 SMTP 邮件',attr:{value:'smtp-email'}}); kind.createEl('option',{text:en?'Workflow':'自动化流程',attr:{value:'workflow'}}); kind.createEl('option',{text:en?'Append to daily note':'追加到今日日记',attr:{value:'append-daily'}}); kind.createEl('option',{text:en?'Create a todo':'创建待办',attr:{value:'create-todo'}}); kind.createEl('option',{text:en?'Push notification':'推送通知',attr:{value:'push'}}); kind.value=draft.kind;
  const commandWrap=field(en?'Command':'命令'); const commandLabel=commandWrap.querySelector('.'+PLUGIN_ID+'-scheduler-label'); const command=commandWrap.createEl('textarea',{attr:{rows:'5',maxlength:'12000'}}); command.value=draft.command;
  ...
  const workflowPicker=commandWrap.createEl('select'); workflowPicker.createEl('option',{text:en?'Choose a workflow…':'选择自动化流程…',attr:{value:''}});
  workflowOptions.forEach((item)=>workflowPicker.createEl('option',{text:item.name,attr:{value:item.id}}));
  if(draft.kind==='workflow'&&draft.command&&!workflowOptions.some((item)=>item.id===draft.command))workflowPicker.createEl('option',{text:(en?'Unavailable: ':'已失效：')+draft.command,attr:{value:draft.command,disabled:'disabled'}});
  workflowPicker.value=draft.kind==='workflow'?draft.command:''; workflowPicker.onchange=()=>{if(workflowPicker.value)command.value=workflowPicker.value;};
  const smtpAccountWrap=field(en?'Sending account':'发件账户'); const smtpAccount=smtpAccountWrap.createEl('select'); smtpAccount.createEl('option',{text:en?'Choose an SMTP account…':'选择 SMTP 发件账户…',attr:{value:''}});smtpAccounts.forEach((item)=>smtpAccount.createEl('option',{text:item.name+' · '+item.email+(item.enabled?'':(en?' (disabled)':'（已停用）')),attr:{value:item.id}}));smtpAccount.value=draft.smtp.accountId;
  const smtpToWrap=field(en?'Recipients':'收件人');const smtpTo=smtpToWrap.createEl('textarea',{attr:{rows:'2',placeholder:en?'name@example.com, team@example.com':'name@example.com，多个地址用逗号分隔'}});smtpTo.value=draft.smtp.to.join(', ');
  const smtpCcWrap=field(en?'CC / BCC':'抄送 / 密送');const smtpCc=smtpCcWrap.createEl('input',{attr:{type:'text',placeholder:en?'CC addresses (optional)':'抄送地址（可选）'}});smtpCc.value=draft.smtp.cc.join(', ');const smtpBcc=smtpCcWrap.createEl('input',{attr:{type:'text',placeholder:en?'BCC addresses (optional)':'密送地址（可选）'}});smtpBcc.value=draft.smtp.bcc.join(', ');
  const smtpSubjectWrap=field(en?'Email subject':'邮件主题');const smtpSubject=smtpSubjectWrap.createEl('input',{attr:{type:'text',maxlength:'180',placeholder:en?'Supports {date} {time} {datetime}':'支持 {date} {time} {datetime}'}});smtpSubject.value=draft.smtp.subject;
  const smtpBodyWrap=field(en?'Email body':'邮件正文');const smtpBody=smtpBodyWrap.createEl('textarea',{attr:{rows:'5',maxlength:'10000',placeholder:en?'Supports {date} {time} {datetime}':'支持 {date} {time} {datetime}'}});smtpBody.value=draft.smtp.body;
  const scheduleType=field(en?'Schedule':'运行计划').createEl('select'); [['interval',en?'Interval':'按间隔'],['daily',en?'Daily':'每天'],['weekly',en?'Weekly':'每周'],['event',en?'On event':'事件触发']].forEach(([value,label])=>scheduleType.createEl('option',{text:label,attr:{value}})); scheduleType.value=draft.schedule.type;
  ...
  const smtpFields=[smtpAccountWrap,smtpToWrap,smtpCcWrap,smtpSubjectWrap,smtpBodyWrap];smtpFields.forEach((item)=>item.classList.add(PLUGIN_ID+'-scheduler-smtp-field'));
  const syncVisibility=()=>{const shell=kind.value==='shell';const obsidianCommand=kind.value==='obsidian-command';const isTemplate=SCHEDULED_TEMPLATE_KINDS.includes(kind.value);const isSmtp=kind.value==='smtp-email';const isEvent=scheduleType.value==='event';const isCountdownEvent=isEvent&&(eventSel.value==='countdown-threshold'||eventSel.value==='countdown-finished');commandWrap.style.display=isSmtp?'none':'';command.style.display=shell||isTemplate?'':'none';commandPicker.style.display=obsidianCommand?'':'none';toolbarPicker.style.display=kind.value==='toolbar-action'?'':'none';workflowPicker.style.display=kind.value==='workflow'?'':'none';smtpFields.forEach((item)=>{item.style.display=isSmtp?'':'none';});if(commandLabel){const labels={shell:[en?'Command':'命令'],'obsidian-command':[en?'Command':'命令'],'toolbar-action':[en?'Command':'命令'],'append-daily':[en?'Diary text':'日记内容'],'create-todo':[en?'Todo text':'待办文本'],push:[en?'Message':'消息内容'],'workflow':[en?'Workflow':'流程']};commandLabel.textContent=(labels[kind.value]||labels.shell)[0];}command.placeholder=isTemplate?(kind.value==='append-daily'?(en?'Text appended to today\'s daily note. Supports {date} {time} {datetime}':'追加到今日日记的文本，支持 {date} {time} {datetime}'):(kind.value==='create-todo'?(en?'Todo text. Supports {date} {time} {datetime}':'待办文本，支持 {date} {time} {datetime}'):(en?'Push message. Supports {date} {time} {datetime}':'推送消息，支持 {date} {time} {datetime}'))):'';interval.parentElement.style.display=!isEvent&&scheduleType.value==='interval'?'':'none';time.parentElement.style.display=!isEvent&&scheduleType.value!=='interval'?'':'none';days.parentElement.style.display=scheduleType.value==='weekly'?'':'none';eventSel.parentElement.style.display=isEvent?'':'none';eventFolder.parentElement.style.display=isEvent&&eventSel.value==='file-saved'?'':'none';eventSource.parentElement.style.display=isCountdownEvent?'':'none';missed.parentElement.style.display=isEvent?'none':'';trusted.parentElement.style.display=shell?'':'none';}; kind.onchange=syncVisibility;scheduleType.onchange=syncVisibility;eventSel.onchange=syncVisibility;syncVisibility();
  save.onclick=async()=>{error.setText('');const isTemplate=SCHEDULED_TEMPLATE_KINDS.includes(kind.value);const selectedCommand=kind.value==='shell'||isTemplate?command.value.trim():(kind.value==='toolbar-action'?toolbarPicker.value:(kind.value==='workflow'?workflowPicker.value:(kind.value==='obsidian-command'?commandPicker.value:'')));const selectedCountdown=countdownOptions.find((item)=>item.id===eventSource.value);const sourceLabel=eventSource.value?(selectedCountdown?.name||draft.schedule.sourceLabel||''):'';const smtp=kind.value==='smtp-email'?normalizeScheduledSmtpMail({accountId:smtpAccount.value,to:smtpTo.value,cc:smtpCc.value,bcc:smtpBcc.value,subject:smtpSubject.value,body:smtpBody.value}):null;const task={...draft,name:name.value.trim(),kind:kind.value,command:selectedCommand,smtp,enabled:enabledBox.checked,trusted:trustBox.checked,schedule:scheduleType.value==='event'?{type:'event',event:eventSel.value,folder:eventFolder.value.trim(),sourceId:eventSource.value,sourceLabel}:{type:scheduleType.value,intervalMinutes:Number(interval.value),time:time.value,weekdays:days.value.split(',').map(Number)},missedPolicy:missed.value,updatedAt:new Date().toISOString()};if(!task.name||(task.kind==='smtp-email'?!task.smtp:!task.command)){error.setText(task.kind==='smtp-email'?(en?'Choose a sending account and enter at least one valid recipient.':'请选择发件账户，并填写至少一个有效收件邮箱。'):(en?'Name and action are required.':'名称和执行动作不能为空。'));return;}if(task.kind==='smtp-email'&&!smtpAccounts.some((item)=>item.id===task.smtp.accountId&&item.enabled)){error.setText(en?'Choose an enabled SMTP account.':'请选择一个已启用的 SMTP 发件账户。');return;}if(task.kind==='obsidian-command'&&!commandPicker.value){error.setText(en?'Choose an available app command.':'请选择一个可用的应用命令。');return;}if(task.kind==='toolbar-action'&&!toolbarActions.some((action)=>action.id===toolbarPicker.value)){error.setText(en?'Choose an available Toolbar action.':'请选择一个当前可用的 Toolbar 动作。');return;}if(task.kind==='workflow'&&!workflowOptions.some((item)=>item.id===workflowPicker.value)){error.setText(en?'Choose an available workflow.':'请选择一个可用的自动化流程。');return;}if(task.kind==='shell'&&task.enabled&&!task.trusted){error.setText(en?'Confirm Shell permission before enabling.':'启用 Shell 任务前请确认权限。');return;}save.disabled=true;try{await view._plugin.scheduledTasks.upsert(task);overlay.remove();}catch(e){error.setText((en?'Could not save: ':'保存失败：')+(e?.message||e));save.disabled=false;}};
