...
class CockpitTeamEditorModal extends obs.Modal {
  constructor(app, service, record = null, initial = null) { super(app); this.service = service; this.record = record; this.initial = initial; }
  onOpen() {
    ...
    const PID = PLUGIN_ID;
    this.modalEl?.addClass('cockpit-team-task-dialog');
    // The task editor supplies the same header close control as the personal todo
    // editor. Remove the host modal's own close button so host-version-specific
    // positioning cannot leave two controls visible.
    this.modalEl?.querySelectorAll?.('.modal-close-button').forEach(button => button.remove());
    this.contentEl.addClass(PID + '-todo-editor-sheet');
