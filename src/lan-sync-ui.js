...
async function renderLanSyncSettings(container, plugin, language) {
  ...
  const refresh = () => {
    ...
    peers.forEach(peer => {
      ...
      const labels = { todos:en?'tasks':'待办', bookmarks:en?'bookmarks':'收藏', 'display-name':en?'display name':'昵称', language:en?'language':'语言', 'morning-brief-routing':en?'morning brief sender':'晨报发送设备' };
