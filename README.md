...
Cockpit Dashboard 是一个面向 Obsidian Vault 的驾驶舱式首页插件，用来替代默认首页，集中展示待办、日历、统计、最近文件、收藏、搜索和专注计时等信息，并支持可持久化的模块布局编辑。

Cockpit Dashboard is a cockpit-style homepage plugin for Obsidian. It replaces the default home view with a focused dashboard for tasks, calendar, stats, recent files, bookmarks, search, focus tracking, and persistent layout customization.
...
| 🧩 编辑模式 | 支持模块上下拖动排序、隐藏/恢复模块，并持久化保存布局 |
...
| 🧩 Edit Mode | Drag modules to reorder them, hide/show sections, and persist your dashboard layout |
