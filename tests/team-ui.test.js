...
(async()=>{
  ...
  assert.ok(editor.overlay?.classes.has('cockpit-dashboard-todo-editor-backdrop'),'Team editor uses the personal-todo backdrop structure');
  assert.equal(editor.contentEl.all().filter(el=>el.classes.has('cockpit-dashboard-todo-editor-close')).length,1,'The team editor keeps one custom close control');
  ...
  reopened.close();
  assert.equal(reopened.overlay,null,'Closing the editor removes its backdrop');
