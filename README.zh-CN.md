...
- **仪表盘工作台**：待办今天队列、精细化日历看板（月/周视图、拖拽排期、农历与法定节假日调休显示、可开关），以及按日期聚合待办、定时/事件自动化、具体执行动作与状态、RSS 摘要的「今日运行总览」；独立的今日 Agenda 时间流继续保留，并提供统计卡片、编辑热力图、项目进度条、分类卡片、最近更新、收藏文件、闪念胶囊、旧笔记重现；面板数据基于库内事件驱动刷新，文件保存后数秒内自动跟随
- **Apple 日历同步（仅 Mac）**：在新建或编辑待办时，可逐项勾选“同步到 iPhone 日历”；首次勾选会请求系统权限，并在默认日历账户中自动复用或创建专用的“Cockpit”日历，无需预先进入设置。勾选且有截止日期的待办会单向写入该日历，仅日期待办成为全天事件，带时间待办成为 30 分钟事件，再由 iCloud 同步到 iPhone
...
- **自动化**：番茄钟（可关联待办）、全局闹钟（后台全屏提醒 + 手势追赶）、适合大量数据的紧凑倒计时卡片，以及百分比/固定剩余时间阈值和多渠道通知；倒计时到达阈值或结束时可直接联动自动化流程、SMTP 邮件及其他任务动作。定时任务还支持指定文件夹保存笔记、待办完成、番茄钟结束等事件，可自动追加日记、创建待办、推送通知、运行 Toolbar 动作 / 应用命令 / 桌面端 Shell / 自动化流程；自动化流程提供多步骤运维流水线（条件跳过、失败重试、输出传递 `{{last_output}}`、失败推送、运行日志落盘），并可从磁盘空间 / 网站可达性 / 备份新鲜度等巡检模板一键创建
- **周报工坊**：多条周报配置以整行可点击的折叠卡片管理；手动生成后可并排对照脚本原稿与流式 AI 优化稿，也能把同一次优化带到 Agent 继续处理，最终由用户明确选择保存版本；周报保存后还能触发现有定时任务事件链路
- **专注分析**：专注趋势图表（折线/柱状），本周对比上周、黄金专注时段、任务投入 Top3 洞察，一键生成周报分享卡片 PNG
- **布局系统**：通过带搜索、分类和组件预览的组件商店添加、隐藏与排序模块，不再让编辑控件覆盖实时仪表盘；原有模块顺序、显隐和折叠状态无感保留，多情景布局可按星期/时间段/文件夹自动切换
...
| 插件 `data.json` | Storage V2 设置、布局情景、Toolbar、RSS 配置、Apple 日历目标与受管事件 UID 映射、AI 模型配置（**API Key 以明文保存在此处**） |
| 插件私有 `ai-history.json` | 最多 30 个 AI 会话（不含附件正文/RAG 片段/思考过程） |
| 插件私有 `ai-index.json` | 倒排索引快照 |
| IndexedDB | 设备本地的 RSS 正文缓存与已读状态 |

无遥测、无后台上传。Apple 日历同步仅 Mac 可用且不会把日历数据发给插件作者：只有你逐项勾选同步时，固定的本机自动化脚本才会请求权限，自动复用或创建专用日历，并把有截止日期的未完成待办写入其中；`data.json` 仅保存目标日历名称与受管事件 UID，供安全更新和删除。只有你主动发起 AI 请求时，所选笔记片段、附件、RAG 命中的截断片段，或你明确点击优化的周报草稿才会发送到你配置的模型服务；自由问答在本机完成检索，不会整体上传 Vault。周报 AI 优化始终保留脚本原稿，另行流式生成可编辑版本，并且只保存你最终选择的版本；原稿与优化稿的限长片段会保存在插件私有 AI 历史中，便于在 Agent 继续同一次请求。Agent 的库内笔记工具只会创建、追加、移动笔记或添加行内标签（绝不删除），且每次修改都需你在对话中逐条确认；只读权限模式下这些工具完全不可见。倒计时和定时任务消息只会发送到你启用的通知渠道；可选邮件渠道会把通知标题、正文、发件人与收件人地址发送给 Resend，其 API Key 与地址以明文保存在插件配置中。自动化流程的运行历史保存在本机配置文件中（每流程最近 20 条）；若你为流程填写了「运行日志笔记」，运行摘要会追加到你指定的那篇笔记；失败推送同样只发送到你已启用的推送渠道。
