...
- **重复待办**：个人待办编辑器可设置每隔若干天、周、月或年重复，选择固定排期或完成后间隔；完成记录独立保留，只生成一条下一次待办，固定排期跳过错过的周期并保留月底日期。编辑下一条未完成待办可调整规则，选择「不重复」停止后续生成
...
- **晨间简报**：定时汇总今日、逾期、团队待办、习惯和专注数据；多台已配对设备可指定其中一台负责发送以避免重复，也可明确选择每台设备都发送
...
从仪表盘「团队待办」、同步设置中的「创建 / 加入 / 管理团队」，或命令面板「打开团队空间」进入。选一台常用电脑创建团队作为主设备；其他电脑扫描、导入或粘贴主设备邀请。主设备批准每台设备，并配置协作/只读角色、全部/仅分配给该设备的可见范围、是否同步团队待办，以及创建、编辑内容、更新完成状态、转派负责人和删除权限。编辑、完成与删除仍只作用于分配给该成员的待办；转派需由主设备单独授权，且只能选择仍在团队中的成员。只读成员不能写入；所有网络操作及离线排队操作都会重新校验当前权限。

个人待办默认不加入团队。「分享个人待办」仅把所选一条复制到团队，保存前可检查内容；原个人待办保留，后续修改相互独立。团队卡片显示来源设备、最后修改设备、负责人、优先级、截止日期与同步状态；经主设备转发不会改变原始来源。模块兼容排序、显示/隐藏、持久化折叠、编辑模式和情景布局。团队待办暂不接入个人统计、日历导出、AI 上下文或完成自动化。

主设备在线时，成员每 30 秒拉取一次，保存后也会立即尝试同步。主设备离线时成员可查看缓存、暂存有权修改的内容；并发修改保留为成员草稿和主设备冲突，由主设备明确选择版本。离线期间权限被收回的修改保留为本机草稿。缩小可见范围会在下次连接时移除对应团队缓存，不会删除主设备待办。移除成员只阻止后续连接，无法收回已下载副本。第一版不做自动主设备切换；地址变化后用新邀请重新配对。每台设备一个团队，最多 8 台成员设备、400 条含删除标记的待办记录，待提交/草稿/冲突各最多 100 项；达到限制时停止写入，不静默丢弃记录。

团队记录单独保存在插件目录 `team-sync-<machine>.json`，已有个人同步不变，也不自动导入个人数据。库内的 `_data/team-todos.md` 同步维护为团队待办的可读镜像，便于搜索、检查和备份；来源设备、权限、版本和冲突仍以私有记录为准，避免手工改写时绕过授权。`.next` 是完整恢复日志，另保留五份 `.backup-N.json` 轮换备份。恢复前先暂停所有电脑的团队同步，保存当前主文件和日志，再选择一致的备份；主设备恢复过旧版本会触发成员版本校验。退出会先备份再清空本机活动团队；需要长期保留的备份请另行复制，避免轮换覆盖。

### 数据与存储原则

| 存储 | 内容 |
|---|---|
| `_data/todos.md` | 待办（行内 `id:`/`due:`/`p:` 元数据与 `repeat:` 重复规则、系列和下一次记录标识，插件自动维护；旧待办无需转换） |
| `_data/team-todos.md` | 团队待办的可读镜像；AI 索引与库内工具不读取该文件，只能通过专用团队上下文选项发送当前可见任务 |
| `_data/focus.md` | 按日期累计的专注历史 |
| 插件 `team-sync-<machine>.json`、`.next` 与备份 | 可选团队同步：团队待办、设备名称和来源 ID、权限、明文保存的本机配对密钥、离线修改与冲突；只通过 AES-256-GCM 加密的局域网连接发送给批准的主设备/成员，个人待办、昵称、语言、收藏、笔记正文和 API Key 不参与 |
| 插件 `data.json` | Storage V2 设置、布局情景、Toolbar、RSS 配置、晨报发送策略、Apple 日历目标与受管事件 UID 映射、AI 模型配置（**API Key 以明文保存在此处**） |
...
- 点击标签项目可查看待办、逾期任务、专注时长及同标签笔记，直接编辑或启动关联番茄钟；已完成项目可切换回顾。
- 在当前筛选中选择任务，批量调整日期、优先级、标签，完成、删除或归档已办任务；在“已归档”筛选中取消归档。归档通过 `_archived` 标签保留在原待办 Markdown 文件中。撤销保留最近 20 次已保存修改，总计不超过 1 MiB，仅存于本次运行内存；相关任务有后续修改时会拒绝覆盖。
- 在编辑布局中开启“团队概览”“运行记录”“同步诊断”，统一支持排序、显示隐藏、折叠和情景布局。团队概览按当前权限统计负责人负载，可导出当前可见的未完成到期任务为本地 ICS 日历；团队完成自动化只在主设备通过授权并落盘后触发。
- AI 中主动勾选专用团队上下文时，才会把当前权限范围内的团队任务摘要发送到配置的模型服务，最多 12000 字符；切换对话后自动取消，不自动检索团队镜像。
- 运行记录汇总本机定时任务日志、流程历史、简报与推送回执，以及最近一次系统日历同步结果。重试只针对选中的任务或渠道，使用当前配置和当前待办重新组装；流程重跑会重新执行整个流程。同步诊断展示对端版本、最后成功时间、待提交修改、草稿和冲突，可进入管理页处理。
