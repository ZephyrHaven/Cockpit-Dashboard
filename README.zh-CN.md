...
从仪表盘「团队待办」、同步设置中的「创建 / 加入 / 管理团队」，或命令面板「打开团队空间」进入。选一台常用电脑创建团队作为主设备；其他电脑扫描、导入或粘贴主设备邀请。主设备批准每台设备，并配置协作/只读角色、全部/仅分配给该设备的可见范围、创建与删除权限，以及是否同步团队待办。协作成员只能修改分配给自己的待办，只有主设备能调整负责人；所有网络写入都重新校验当前权限。
