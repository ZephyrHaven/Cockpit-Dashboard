...
- **仪表盘工作台**：待办今天队列、日历看板（月/周视图、拖拽排期、农历与法定节假日调休显示、可开关），以及按日期聚合待办、定时/事件自动化、具体执行动作与状态、RSS 摘要的「今日运行总览」；独立的今日 Agenda 时间流继续保留，并提供统计卡片、编辑热力图、项目进度条、分类卡片、最近更新、收藏文件、闪念胶囊、旧笔记重现；面板数据基于库内事件驱动刷新，文件保存后数秒内自动跟随
...
- **自动化**：番茄钟（可关联待办）、全局闹钟（后台全屏提醒 + 手势追赶）、可配置百分比/固定剩余时间阈值和多渠道通知的倒计时；倒计时到达阈值或结束时可直接联动自动化流程及其他任务动作。定时任务还支持指定文件夹保存笔记、待办完成、番茄钟结束等事件，可自动追加日记、创建待办、推送通知、运行 Toolbar 动作 / 应用命令 / 桌面端 Shell / 自动化流程；自动化流程提供多步骤运维流水线（条件跳过、失败重试、输出传递 `{{last_output}}`、失败推送、运行日志落盘），并可从磁盘空间 / 网站可达性 / 备份新鲜度等巡检模板一键创建
