...
- 多台已配对电脑现在可以指定其中一台负责发送晨报，避免同一内容重复推送；也可选择所有设备都发送，并分别保留各设备的发送记录。

请在所有参与设备上更新并刷新插件，然后前往 **Cockpit 设置 → 晨间简报 → 多设备发送** 选择发送电脑；旧版本设备无法识别这项路由设置。

## 1.8.10 更新

- 晨报默认汇总当前可见的团队待办；单独的团队待办计划提醒改为显式开启，避免早上连续收到两条内容重叠的通知。晨报和独立提醒都按清晰分区展示，一条待办占一行。
- 日历中的团队待办与个人待办统一为全天日期、团队来源标签和独立标签胶囊，不再显示没有实际意义的午夜时间。
- 团队成员权限拆分为创建、编辑内容、更新完成状态、转派负责人和删除，主设备会对实时及离线排队操作逐项裁决；成员卡片同步重做分组、说明和有效权限摘要。

升级后，已有协作成员继续保留原来的创建、编辑、完成和删除权限；新增的转派权限默认关闭。旧版含义模糊的团队通知开关不会自动开启额外的独立提醒。

## 1.8.9 更新

- 团队待办沿用个人待办的紧凑列表、标签、优先级和精确到秒的截止时间交互；新增与编辑弹窗统一只显示一个关闭按钮。
- 团队待办单独镜像到 `_data/team-todos.md`，在日历中分项显示并保留来源设备；晨间简报和消息推送可按独立开关选择是否包含团队待办。
- 团队空间支持主设备管理角色、可见范围和同步权限，设备授权后加密同步，并提供设备名称唯一校验、离线草稿与冲突处理。
